...
function buildvRAReservationParam ($theName, $theBusinessGroup, $theComputeResource, $thePlatformType, $theNetworkPath, $theNetworkProfile, $theDataStorePath) {
	...
	$NetworkDefinitionArray += $myNetwork

	$StorageDefinitionArray = @() 
	$myStorage = New-vRAReservationStorageDefinition -Type $thePlatformType -ComputeResourceId $myComputeResource.Id -Path $theDataStorePath -ReservedSizeGB $GLOBAL:STORAGESIZEGB -Priority $GLOBAL:STORAGEPRIORITY
	$StorageDefinitionArray += $myStorage
